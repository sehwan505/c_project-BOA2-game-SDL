		...
		else
		{
			...
			while (!quit)
			{
				...
				while (SDL_PollEvent(&e) != 0)
				{
					...
					else if (e.type == SDL_KEYDOWN) //키다운 이벤트(함수로 처리할것)
					{
				...
				render(&gBackgroundTexture, gRenderer, 0, 0);//배경넣기
				SDL_RenderFillRect(gRenderer, &wall);  //벽만들기

				render(&gCurrentSurface, gRenderer, gPlayer.mPosX, gPlayer.mPosY); //플레이어 무브
...
void move(_LPlayer* LP,SDL_Rect* wall) //플레이어 무브 & 충돌처리
{
	...
	if ((LP->mPosX < 0) || (LP->mPosX + LP->Player_WIDTH > SCREEN_WIDTH) || checkCollision(LP->mCollider,*wall))
	{
		LP->mPosX -= LP->mVelX;
		LP->mCollider.x = LP->mPosX;
	}
	...
	if ((LP->mPosY < 0) || (LP->mPosY + LP->Player_HEIGHT> SCREEN_HEIGHT*2/3) || checkCollision(LP->mCollider,*wall))
	{
		LP->mPosY -= LP->mVelY;
		LP->mCollider.y = LP->mPosY;
	}
}
void T_handleEvent(_LTexture* CT, _LTexture* LT[], SDL_Event* e) //오류 찾을것 (아마 구조체 배열 접근에서 에러있는거 같음)
{
