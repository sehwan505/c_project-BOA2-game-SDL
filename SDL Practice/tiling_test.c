...
void Tile_render(SDL_Rect* camera, _LTile* tiles, _LTexture* LT, SDL_Renderer* Renderer);
	...
	else
	{
		...
		else
		{
			...
			while (!quit)
			{
				...
				{
					Tile_render(&Camera, &tileSet, &gTileTexture, &gRenderer);
				}
...
void Tile_render(SDL_Rect* camera, _LTile* tiles, _LTexture* LT, SDL_Renderer* Renderer)
{
	if (checkCollision(*camera, tiles->mBox))
	{
		render((LT+(tiles->mType))->mTexture, Renderer, tiles->mBox.x - camera->x, tiles->mBox.y - camera->y);
	}
}